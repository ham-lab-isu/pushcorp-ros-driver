...
request_string = "/afd/actualForce"

class AFDForcePublisher(Node):

    def __init__(self):
        super().__init__('afd_force_publisher')
        ...
        self.publisher_ = self.create_publisher(Value, 'pushcorp/afd/actualForce', 10) 
    ...
    def publish_udp_data(self):
        ...
        actual_force = response.decode()
        udp_socket.close()

        # Extract the numerical value from the UDP data
        position_value = json.loads(actual_force)['data']['/afd/actualForce']
...
def main(args=None):
    ...
    afd_force_publisher = AFDForcePublisher()
    rclpy.spin(afd_force_publisher)
    afd_force_publisher.destroy_node()
    rclpy.shutdown()
